...
### ⑥ 締め（CTA）
素材メモの内容からA・Bどちらか適切な方を自動選択する。

パターンA（年齢訴求｜運動・代謝系テーマの時）：
...
パターンB（引き締め訴求｜食事・継続系テーマの時）：
よくSNSでは食事だけでキレイに痩せますと言われていますが
35歳以上の女性の場合は
筋肉の張りがないと重力に負けてボディラインはたるみます。
ただ痩せるだけではなく引き締まった体が欲しい女性は
ぜひ続けて頑張ってみてくださいね
もっと痩せたい女性のために
プロフィールリンクからお家で痩せる9大特典をプレゼント中です！
...
【文字数】850〜950文字（セリフのみ。最後に実際の文字数を括弧で記載する）"""
...
【出力】台本本文のみ出力すること（説明・コメント不要）"""
            try:
                ...
                pass
        # YouTubeはCTAを自動追記
        if script_type == "youtube" and draft and not draft.startswith("（生成エラー"):
            draft = draft.rstrip() + "\n\n" + YOUTUBE_CTA
        return draft
    except Exception as e:
        return f"（生成エラー: {e}）"
