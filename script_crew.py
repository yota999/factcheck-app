...
# ── LLM呼び出し（LiteLLM経由でClaude Sonnet 4.6を使用） ──────────────
def _call_llm(prompt: str, temperature: float = 0.7, max_tokens: int = 8000) -> str:
    """Claude Sonnet 4.6にプロンプトを送って結果を返す"""
    import litellm
...
def get_trends(keyword: str) -> list:
    ...
    try:
        ...
        resp = requests.post(
            ...
            json={"q": keyword, "gl": "jp", "hl": "ja", "num": 5},
...
1. オープニング：挨拶とテーマ発表、視聴者の悩みに深く寄り添う
...
【文字数】700〜800文字（セリフのみ。最後に実際の文字数を括弧で記載する）"""
...
def generate_themes(
    ...
    used_str = "\n".join(f"・{t}" for t in used_themes[-20:]) if used_themes else "（まだなし）"
    trend_str = "\n".join(trends) if trends else "（取得できませんでした）"

    prompt = f"""{persona}

キーワード「{keyword}」に関連する{char_range}のテーマを20個提案してください。

【アングル（切り口）】「{angle_name}」を軸に展開すること

【使用済みテーマ（これと被らないこと・似たものも避ける）】
{used_str}

【最新トレンド情報（参考にして新鮮なテーマを作る）】
{trend_str}
...
20. テーマタイトル"""
    ...
    response = _call_llm(prompt, temperature=0.8, max_tokens=2000)
    themes = []
...
def generate_ideas(
    ...
    good_str = "\n".join(f"・{e}" for e in good_elements) if good_elements else "（まだデータなし）"
...
20. アイデア内容"""
    ...
    response = _call_llm(prompt, temperature=0.7, max_tokens=3000)
    ideas = []
    for line in response.split("\n"):
        line = line.strip()
        if not line:
...
def generate_draft(
    ...
    good_str = "\n".join(f"・{e}" for e in good_elements) if good_elements else "（データなし）"
    bad_str = "\n".join(f"・{p}" for p in bad_patterns) if bad_patterns else "（データなし）"

    ref_str = ""
    for i, ref in enumerate(ref_scripts, 1):
        # 参考台本は冒頭800文字だけ渡す（トークン節約）
        ref_str += f"\n【参考台本{i}（冒頭抜粋）】\n{ref[:800]}\n"
...
・セリフのみ（ト書き・カッコ書きの指示は最小限に）"""
    ...
    return _call_llm(prompt, temperature=0.65, max_tokens=max_tok)


def generate_titles(draft: str, script_type: str) -> str:
    """タイトル候補5本 + サムネイルテキスト案3本を生成する"""
    persona = YOUTUBE_PERSONA if script_type == "youtube" else REEL_PERSONA
...
3. キャッチコピー"""
    ...
    return _call_llm(prompt, temperature=0.8, max_tokens=1000)
