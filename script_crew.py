...
【修正のルール】
- ❌「明確に誤り」「そのようなデータは存在しない」「事実として成立しない」と判定された箇所は、その文・センテンスごと台本から完全に削除する
- ⚠️要注意のうち「具体的な数字・データが実際に間違っている」と指摘された箇所は、正しいデータ・表現に置き換えて修正する
- ⚠️要注意のうち「可能性としてあり得る・科学的に否定しきれない・一定の根拠がある」と判定された箇所は一切修正しない（そのまま残す）
- ✅正確と判定された箇所は変更しない
- 文体・全体の流れは元の台本を維持する（削除により文章がつながらなくなる場合は自然につなぎ直す）
- 修正箇所が分かるよう、変更した部分の説明も最後に箇条書きで添える（削除した箇所は「削除」と明記）

【禁止事項】
- 台本の主張・論旨に対する反論・逆説を新たに追加しない（例：台本が「Aは効果的でない」と主張しているとき、「ただし条件次第ではAも有効」などを足さない）
- ファクトチェックの指摘文言をそのまま台本に書き込まない
- 台本全体の主張・結論の方向性を変えない
- ※ただし、間違ったデータを正しいデータに置き換える修正は禁止に該当しない
