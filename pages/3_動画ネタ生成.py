...
# ──────────────────────────────────────────────────────────────
# 視点リスト（リール用：解剖学・筋肉・関節・姿勢テーマ）
# ──────────────────────────────────────────────────────────────
REEL_ANGLES = [
    "内閉鎖筋のジョイントセントレーションと大臀筋・ハムストリングスの活性化",
    "深層外旋六筋のジョイントセントレーションと股関節大筋群への力伝達",
    "多裂筋・腹横筋のコスタビリティと体幹大筋群（脊柱起立筋・腹直筋）の出力",
    "胸腰筋膜の張力伝達と殿筋群・広背筋の筋膜連鎖",
    "アナトミートレイン（筋膜連鎖ライン）が途切れると大筋群が使えなくなる仕組み",
    "仙腸関節の求心位（ジョイントセントレーション）と下肢大筋群の神経筋活性化",
    "前鋸筋の肩甲骨固定と大胸筋・三角筋・僧帽筋の機能分担",
    "腸骨筋と大腰筋の個別機能と大腿四頭筋・ハムストリングスへの影響",
    "肩甲上腕リズムの崩れが肩甲骨周囲大筋群のパフォーマンスを落とす仕組み",
    "大腿筋膜張筋（TFL）の過活動が大臀筋の機能を抑制するメカニズム",
    "腹斜筋の対角線収縮パターンと体幹回旋における大筋群連動",
    "脊椎の分節的安定化（ローカル筋・グローバル筋の分業）と大筋群出力の関係",
    "腓骨筋群の求心位制御と下肢連鎖（大腿四頭筋・ハムストリングス）への影響",
    "関節包拘縮によるジョイントセントレーション破綻と周囲筋群の代償収縮",
    "頸椎アライメントと頭頸部深層筋が体幹大筋群の出力に与える影響",
]
...
・[動画タイトル案：視聴者が思わずクリックする一言。「実は〇〇だった」「〇〇する人が痩せない本当の理由」など直感的に内容がわかる1行]
　内容：[その動画で実際に話す内容を2〜3文で。「〇〇というメカニズムがあり、それが〇〇に影響する」という流れで、専門家目線だが日常語で書く]
　根拠：[裏にある専門的メカニズムを1行で補足。専門用語は括弧で一言補足を入れる]
　→ 例え：[小学生でもわかる日常のたとえで1〜2文]

【タイトル案の書き方ルール】
・視聴者目線で「これ、私のことだ」「知りたい！」と思える表現にする
・専門用語はタイトルに入れない。内容と根拠の欄で説明する
・「実は」「意外と」「〇〇するだけで」「〇〇する人ほど」などの表現を積極的に使う

【内容の書き方ルール】
・動画を企画する人が「この内容なら話せる」とすぐわかるレベルで書く
・専門用語は避けつつ、内容の核心（何が起きているか）はちゃんと伝える
・「〜という仕組みがあって、だから〜になる」という因果の流れを必ず入れる

・日本語のみ
・「内容：」「根拠：」「→ 例え：」は必ず毎回入れること"""

# ──────────────────────────────────────────────────────────────
# システムプロンプト（リール用：解剖学・筋肉・関節・姿勢に特化）
# ──────────────────────────────────────────────────────────────
REEL_SYSTEM_PROMPT = """あなたはSNSリール動画・ショート動画のコンテンツ企画のプロです。
30代〜50代の日本人女性が「体型・姿勢・体の仕組み」をテーマにリール動画で発信するネタを考えます。

【テーマの方向性】
解剖学・筋肉・関節・骨格・筋膜・姿勢に関する話のみ扱う。
「体のつくり（構造）」が体型や動きにどう影響するかを、わかりやすく伝えるネタを考える。

【ターゲット】
・30代〜50代の一般的な日本人女性
・専門知識はない・普通の主婦やワーキングウーマン
・「なんとなく体型が変わってきた」「運動しても変わらない」「なんか体が硬くなった気がする」と感じている人

【絶対に出力禁止】
■ ホルモン・精神・内科系（全て排除）
・エストロゲン・女性ホルモン・更年期・ホルモンバランス
・コルチゾール・ストレス・自律神経・メンタル・マインドフルネス
・腸内環境・消化・代謝・血糖・インスリン
・睡眠・疲労・体温・冷え性・尿もれ
→ 内科的・精神的・ホルモン的なものは全て排除

■ SNS・一般メディアで広まっているありきたりな解剖学系（排除）
・足根骨・ショパール関節・リスフラン関節など足部の骨・関節の話
・足底アーチ・土踏まず（SNSで飽和済み）
・骨盤矯正・骨盤の歪み（SNSで飽和済み）
・猫背・巻き肩矯正（飽和済み）
・体幹トレーニング全般（飽和済み）
・インナーマッスルを鍛えましょう（飽和済み）
・ストレッチで柔軟性を上げましょう（飽和済み）
・O脚・X脚矯正（飽和済み）
→ 整体院・ピラティス・ヨガ系SNSでよく見るような一般的な内容はNG

■ 食事・生活習慣系
・PFCバランス / カロリー計算 / 食事制限 / 有酸素運動
→ 全てNG

【出力すること — 必ずこのパターンで構成する】

◎ 推奨パターン：「深層の専門概念 → 大筋群が使える状態になる → 体型・脂肪燃焼への効果」

例：内閉鎖筋を活性化させてジョイントセントレーション（関節の最適な求心位）を整えると、
　　大臀筋・ハムストリングス・大腿四頭筋といった脂肪燃焼効率の高い大筋群が正しく働ける状態になる

■ 「深層の専門概念」として使えるもの（一般人はほぼ知らない）
・ジョイントセントレーション（関節の求心位）
・筋膜連鎖・アナトミートレイン
・ローカル筋・グローバル筋の分業システム
・深層外旋六筋・内閉鎖筋・多裂筋など深層安定筋
・関節包拘縮・代償収縮
・肩甲上腕リズム・胸腰筋膜張力

■ 「大筋群」として登場させてよいもの（一般人も知っている）
・大臀筋・中臀筋・ハムストリングス・大腿四頭筋
・大胸筋・広背筋・三角筋・僧帽筋
・脊柱起立筋・腹直筋・腹斜筋
...
・専門用語は積極的に使う（必ず括弧で一言補足を入れる）
・解剖学・運動学的な根拠があること
・一般人が「なぜ運動しても変わらないか」の答えになるような内容

【出力形式】
必ず以下の形式で3〜5個出すこと：

・[フック：思わずスクロールを止める、最初の1〜2秒で興味を引く一言タイトル]
　ひとこと解説：[そのネタが「何の筋肉・関節・骨格の話か」を2文で。日常語で]
　→ 例え：[小学生でもわかる、もっと身近な例え話で2〜3文]

【ひとこと解説の書き方ルール】
・専門用語は残してよいが、括弧で一言補足　例：「腸腰筋（腰の奥にある筋肉）」
・「これは〇〇（筋肉名/関節名）の話で、〜という理由で体型に影響します」のように書く
・「なるほど、そういう仕組みか！」と思えるレベルで
...
def format_content(text: str) -> str:
    ...
    while i < len(lines):
        ...
        elif line.startswith(("ひとこと解説", "内容", "根拠")):
