...
# ──────────────────────────────────────────────────────────────
REEL_ANGLES = [
    "アナトミートレイン（筋膜連鎖ライン）と体型・姿勢の変化",
    "仙腸関節の微小運動と骨盤の安定・不安定メカニズム",
    "胸腰筋膜の張力伝達と体幹剛性の仕組み",
    "深層外旋六筋（股関節の奥の6つの筋肉）と股関節機能",
    "多裂筋（脊椎の最深部の安定筋）と腰部アライメント",
    "腸骨筋と大腰筋の個別機能の違いと骨盤への影響",
    "肩甲上腕リズムと肩甲骨・上腕骨の連動メカニズム",
    "後脛骨筋と足底内側アーチの制御機構",
    "腹斜筋の対角線的収縮パターンと体幹の回旋安定性",
    "恥骨結合の可動性と骨盤剛性の関係",
    "大腿筋膜張筋（TFL）と腸脛靭帯の張力制御",
    "頸椎の前弯角と頭部重心位置が全身姿勢に与える影響",
    "関節包（カプセル）の拘縮と関節可動域制限のメカニズム",
    "脊椎分節的安定化と局所・大域筋システムの役割分担",
    "腓骨筋群（長・短腓骨筋）と足関節外側安定性・重心制御の仕組み",
]
...
【出力すること】
・解剖学・運動学・徒手療法の専門家（理学療法士・整骨院・アスレチックトレーナーなど）は知っているが、
　一般の女性はほぼ知らない、筋膜・関節・骨格・筋肉の「構造的な仕組み」に関する話
・「アナトミートレイン」「筋膜連鎖」「関節包拘縮」のような、専門書や解剖学の授業で出てくるレベルの話
・「体のその部位がそういう動きをしていたなんて知らなかった！」という驚きがある内容
・専門用語を使う（ただし括弧で一言補足を必ず入れる）
・解剖学・運動学的な根拠があること
